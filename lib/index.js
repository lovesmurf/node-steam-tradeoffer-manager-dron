...
TradeOfferManager.prototype.getSendedOffersNonApi = function (options, callback) {
	console.log(this.steamID.getSteamID64());
	this._requestCommunity.get({
		uri: communityURL + `/profiles/${this.steamID}/tradeoffers/sent/?history=1`
	}, function (err, response, body) {
		if (err) {
			return callback(err);
		}
		...
		for (let i = 0; i < offers.length; i++) {
			...
			// const givens = offer('div.tradeoffer_items primary.div.tradeoffer_item_list.div.trade_item');
