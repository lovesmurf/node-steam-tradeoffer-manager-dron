...
TradeOfferManager.prototype.getTradeOffer = function (options, callback) {
	this._community.httpRequestGet({
		uri: communityURL + `/profiles/${this._community.steamID.getSteamID64()}/tradeoffers/sent/?history=1`
...
TradeOfferManager.prototype.cancelOfferByOfferId = function (tradeOfferId, callback) {
	...
	this._community.httpRequestPost({
		...
		headers: {
			referer: communityURL + `/profiles/${this._community.steamID.getSteamID64()}/tradeoffers/sent/`
		},
		json: true,
		form: formFields
...
TradeOfferManager.prototype.getHoldOffersNonApi = function (options, callback) {
	this._community.httpRequestGet({
		uri: communityURL + `/profiles/${this._community.steamID.getSteamID64()}/tradeoffers/sent/`
	}, function (err, response, body) {
		if (err) {
			return callback(err);
		}
...
TradeOfferManager.prototype.getSendedOffersNonApi = function (callback) {
	console.log(communityURL + `/profiles/${this._community.steamID.getSteamID64()}/tradeoffers/sent/?history=1`)
	this._community.httpRequestGet(communityURL + `/profiles/${this._community.steamID.getSteamID64()}/tradeoffers/sent/?history=1`,
		function (err, response, body) {
		...
		for (let i = 0; i < offers.length; i++) {
			...
			let givenItemsHtml = offer(".secondary > .tradeoffer_item_list > .trade_item");
