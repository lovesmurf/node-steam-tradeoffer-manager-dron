...
TradeOfferManager.prototype.getSendedOffersNonApi = function (callback) {
	this._requestCommunity.get({
		...
		for (let i = 0; i < offers.length; i++) {
			...
			let receivedItems = offer('div.tradeoffer_items.primary');
			receivedItems = offer('div.trade_item');

			receivedItems = receivedItems.map(el => {
				return {
					classId: el.attribs['data-economy-item'].split('/')[2]
				}
			});

			let givenItems = offer('div.tradeoffer_items.secondary');
			givenItems = offer('div.trade_item');

			givenItems = givenItems.map(el => {
				return {
					classId: el.attribs['data-economy-item'].split('/')[2]
				}
			})
