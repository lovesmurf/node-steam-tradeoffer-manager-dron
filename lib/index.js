...
const LeastUsedCache = require('@doctormckay/stdlib').DataStructures.LeastUsedCache;
...
TradeOfferManager.prototype.setup = function (options) {
	...
	options.webCookie.forEach(function (name) {
		setCookiev1.bind(this)(name);
	}.bind(this));

	setCookiev1.bind(this)("Steam_Language=english");
};

function setCookiev1(cookie) {
	this._j.setCookie(request.cookie(cookie), communityURL);
}

TradeOfferManager.prototype.getTradeOffer = function (options, callback) {
	this._requestCommunity.get({
		uri: communityURL + `/profiles/${this.steamID.getSteamID64()}/tradeoffers/sent/?history=1`,
		proxy: this.proxy
		...
		let offer = $(`div.tradeoffer#tradeofferid_${options.tradeOfferId}`);

		if (!offer.length) {
			return callback(new Error('Trade Offer Id not found'))
		}

		offer = cheerio.load(offer[0], {decodeEntities: false});
		...
		if (offerState.length > 0) {
			switch (offerState[0].attribs.class) {
				case "tradeoffer_items_banner accepted":
					tradeOfferState = ETradeOfferState.Accepted
					break;

				case "tradeoffer_items_banner inactive":
					tradeOfferState = ETradeOfferState.Declined;
					break;
				default:
					tradeOfferState = ETradeOfferState.Active;
					break;
			}
		}

		callback(null, {
			id: options.tradeOfferId,
			state: tradeOfferState,
		});
	});
};

TradeOfferManager.prototype.cancelOfferByOfferId = function (tradeOfferId, callback) {
	var formFields = {
		sessionid: this.sessionID
	};

	this._requestCommunity.post({
		uri: communityURL + `/tradeoffer/${tradeOfferId}/cancel`,
		headers: {
			referer: communityURL + `/profiles/${this.steamID.getSteamID64()}/tradeoffers/sent/`
		},
		json: true,
		form: formFields
...
TradeOfferManager.prototype.getHoldOffersNonApi = function (options, callback) {
	this._requestCommunity.get({
		uri: communityURL + `/profiles/${this.steamID.getSteamID64()}/tradeoffers/sent/`
	}, function (err, response, body) {
		if (err) {
			return callback(err);
		}
...
TradeOfferManager.prototype.getSendedOffersNonApi = function (callback) {
	this._requestCommunity.get({
		uri: communityURL + `/profiles/${this.steamID.getSteamID64()}/tradeoffers/sent/?history=1`
	}, function (err, response, body) {
		if (err) {
		...
		for (let i = 0; i < offers.length; i++) {
			...
			let receivedItemsHtml = offer('div.tradeoffer_items.primary');
			receivedItemsHtml = offer('div.trade_item');
			let givenItemsHtml = offer('div.tradeoffer_items.secondary');
			givenItemsHtml = offer('div.trade_item');

			const receivedItems = [];
			const givenItems = [];

			for (let d = 0; d < receivedItemsHtml.length; d++) {
				receivedItems.push({ classId: receivedItemsHtml[d].attribs['data-economy-item'] })
			}
			for (let d = 0; d < givenItemsHtml.length; d++) {
				givenItems.push({ classId: givenItemsHtml[d].attribs['data-economy-item'] })
			}

			// const receivedItems = receivedItemsHtml.map((i, index) => {
			// 	const item = cheerio.load(receivedItemsHtml[index], {decodeEntities: false});
			//
			// 	return {
			// 		classId: item.attribs['data-economy-item']
			// 	}
			// })

			//const receivedItems = offer('div.tradeoffer_items_banner.tradeoffer_items primary.tradeoffer_item_list.trade_item');
			//const givenItems = offer('div.tradeoffer_items_banner.tradeoffer_items secondary.tradeoffer_item_list.trade_item');
...
TradeOfferManager.prototype.makeOffer = function (options, callback) {
	var cb = function () {
