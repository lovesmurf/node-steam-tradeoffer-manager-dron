...
TradeOfferManager.prototype.getSendedOffersNonApi = function (callback) {
	this._requestCommunity.get({
		...
		for (let i = 0; i < offers.length; i++) {
			...
			let receivedItemsHtml = $(`div#tradeoffer_${tradeOfferId}.tradeoffer_items_ctn.primary.tradeoffer_item_list.trade_item`);
			let givenItemsHtml = offer('div.tradeoffer_items_ctn.tradeoffer_items.primary.tradeoffer_item_list.trade_item')
			//let givenItemsHtml = offer('div.tradeoffer_items.secondary.tradeoffer_item_list.trade_item')

			console.log(receivedItemsHtml.length, givenItemsHtml.length)
			...
			for (let d = 0; d < receivedItemsHtml.length; d++) {
				receivedItems.push({
					classId: receivedItemsHtml[d].attribs['data-economy-item'].split('/')[2],
					appId: receivedItemsHtml[d].attribs['data-economy-item'].split('/')[1]
				})
			}
			for (let d = 0; d < givenItemsHtml.length; d++) {
				givenItems.push({
					classId: givenItemsHtml[d].attribs['data-economy-item'].split('/')[2],
					appId: givenItemsHtml[d].attribs['data-economy-item'].split('/')[1]
				})
			}
