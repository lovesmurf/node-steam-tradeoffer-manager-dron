...
TradeOfferManager.prototype.getSendedOffersNonApi = function (callback) {
	this._requestCommunity.get({
		...
		for (let i = 0; i < offers.length; i++) {
			...
			let receivedItemsHtml = offer('div.tradeoffer_items.primary.trade_item')

			const receivedItems = [];
			const givenItems = [];

			for (let d = 0; d < receivedItemsHtml.length; d++) {
				console.log({rLength: receivedItemsHtml.length})
				if (receivedItemsHtml.length > 0) {
					console.log(receivedItemsHtml[d].attribs['data-economy-item'])
					receivedItems.push({
