...
TradeOfferManager.prototype.getSendedOffersNonApi = function (callback) {
	this._requestCommunity.get({
		...
		for (let i = 0; i < offers.length; i++) {
			...
			let receivedItems = cheerio.load($(`div.tradeofferid_${tradeOfferId}.trade_item`), {decodeEntities: false});
			receivedItems = receivedItems[i].attribs['data-economy-item'].split('/')
