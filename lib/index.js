...
TradeOfferManager.prototype.getTradeHistory = function (callback) {
	this._community.httpRequestGet(communityURL + `/profiles/${this._community.steamID.getSteamID64()}/tradehistory`,
		function (err, response, body) {
			if (err) {
				return callback(err);
			}
			...
			const $ = cheerio.load(body);
			const trades = $('div.tradehistoryrow');
